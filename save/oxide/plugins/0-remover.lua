...
TableActivedRemove = {}
TableActivedRemoveTimers={}

local function notifyAutoOff(steamid)

local users = rust.GetAllNetUsers()
if(not users) then return end

  for k,v in pairs(users) do
    if(v and rust.CommunityIDToSteamID(tonumber(rust.GetUserID(v))) == steamid) then
        rust.Notice(v, "Удаление автоматически выключено!")
        rust.SendChatToUser(v, "Remover" , "Удаление объектов автоматически отключено в целях безопасности." )
      return
    end
  end
end

local function ManageSafetyTimer(steamid, isDestroy)
  local cachedEntry = TableActivedRemoveTimers[steamid]
  if(cachedEntry) then
  
    if(isDestroy) then
    cachedEntry["Timer"]:Destroy()
    TableActivedRemoveTimers[steamid]=nil
    else
    cachedEntry["Timer"]:Destroy()
    TableActivedRemoveTimers[steamid]={Timer = timer.Once(301, function() TableActivedRemove[steamid]=false TableActivedRemoveTimers[steamid]=nil notifyAutoOff(steamid) end), id = steamid}
    end
  
    elseif not isDestroy then
    -- create new.
    TableActivedRemoveTimers[steamid]={Timer = timer.Once(301, function() TableActivedRemove[steamid]=false TableActivedRemoveTimers[steamid]=nil notifyAutoOff(steamid) end), id = steamid}
  end
end
...
function PLUGIN:ActiveRemove( netuser, cmd, args )
  ...
  if TableActivedRemove[steamID] then
    ...
    ManageSafetyTimer(steamID, true)
    rust.Notice(netuser, "Удаление выключено.")
  else
    TableActivedRemove[steamID] = true
    ManageSafetyTimer(steamID, false)
    rust.Notice(netuser, "Удаление включено! Будьте аккуратны!")
    rust.SendChatToUser( netuser, "Remover" , "Удаление объектов включено!" )
    rust.SendChatToUser( netuser, "Remover" , "Через 5 минут неактивности удаление будет автоматически отключено." )
  end
...
ItemTable["RepairBench(Clone)"] = "Repair Bench"
  ...
  if takedamage then
    ...
    if takedamage.gameObject then
      ...
      if takedamage.gameObject.Name then
        -- Rust.Rust.Notice.Popup( damage.attacker.client.netUser.networkPlayer, "в� ", tostring(takedamage.gameObject.Name), 4.0 )
...
local function cmdGive( netuser, args )
  if (not args[1]) then return  end
  local datablock = rust.GetDatablockByName( args[1] )
  if (not datablock) then
    rust.Notice( netuser, "Remover: No such item!" )
    return
  end
  local amount = tonumber( args[2] ) or 1
  -- for a,b in pairs(Rust) do  rust.BroadcastChat("test", tostring(a) .. " => " .. tostring(b)) end
  -- rust.RunServerCommand ( "notice.popupall \"" .. tostring(Rust) .. "\"")
  local pref = rust.InventorySlotPreference( InventorySlotKind.Default, true, InventorySlotKindFlags.Belt )
  ...
  if (type( inv.AddItemAmount ) == "string") then
    print( "Remover: AddItemAmount was a string! (inv = " .. tostring( inv ) .. " - " .. (inv and inv:GetType().Name or "") .. ")" )
  else
    inv:AddItemAmount( datablock, amount, pref )
  end
  rust.InventoryNotice( netuser, tostring( amount ) .. " x " .. datablock.name )
  ManageSafetyTimer(rust.CommunityIDToSteamID(tonumber(rust.GetUserID(netuser))), false)
                ...
                if item1.uses >= 5 then
                  ...
                  if item1.uses > 5 then
                    ...
                    timer.Once(0.05, function()  
                      ...
                      cmdGive(netusers, {util.QuoteSafe( "Wood" ), num})
                  ...
                  timer.Once(0.05, function()  
                    local netusers = netuser
                    cmdGive(netusers, {util.QuoteSafe( "Wood" ), item1.uses})
