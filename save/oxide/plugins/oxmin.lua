--[[ **************** ]]--
--[[ oxmin - thomasfn / DefaultPlayer ]]--
--[[ **************** ]]--

-- Доработанный Oxmin. Исправлены баги и кое что улучшено (под себя!).


-- Define plugin variables
PLUGIN.Title = "Oxmin"
PLUGIN.Description = "Administration mod"
PLUGIN.Author = "thomasfn / DefaultPlayer"
PLUGIN.Version = "1.5"
...
function PLUGIN:OnUserConnect( netuser )
	...
	data.LastConnect = System.DateTime.UtcNow:AddHours(4):ToString("dd/M/yyyy HH:mm:ss")
    self:Save()
	...
	if (self.Config.showconnectedmessage) then self:BroadcastChat( netuser.displayName .. " пришел к нам." ) end
end
...
function PLUGIN:OnUserDisconnect( networkplayer )
	...
	if (self.Config.showdisconnectedmessage) then self:BroadcastChat( netuser.displayName .. " уходит от нас." ) end
end
...
function PLUGIN:GetUserDataFromID( userID, name )
	...
	if (not userentry) then
		...
		userentry.FirstConnect = System.DateTime.UtcNow:AddHours(4):ToString("dd/M/yyyy HH:mm:ss")
		userentry.LastConnect = 0
		...
		if (netplayer) then
			...
			if (netuser) then
				if (self:HasFlag( netuser, FLAG_GODMODE, true )) then
					...
					local body = controllable:GetComponent("HumanBodyTakeDamage")
					body:SetBleedingLevel(0)
					local fd = controllable:GetComponent("FallDamage")
					fd:ClearInjury()
					local mt = controllable:GetComponent("Metabolism")
					mt:AddCalories(3000)
					mt:AddAntiRad(3000)
...
function PLUGIN:cmdBring( netuser, args )
	if (not args[1]) then
		rust.Notice( netuser, "Syntax: /bring target" )
		return
	end
	local b, targetuser = rust.FindNetUsersByName( args[1] )
	if (not b) then
	...
	rust.ServerManagement():TeleportPlayerToPlayer( targetuser.networkPlayer, netuser.networkPlayer )
	rust.Notice( targetuser, "You teleported to '" .. util.QuoteSafe( netuser.displayName ) .. "'!" )
	rust.Notice( netuser, "'" .. util.QuoteSafe( targetuser.displayName ) .. "' teleported to you!" )
...
-- local RaycastAll = util.GetStaticMethod(UnityEngine.Physics,"RaycastAll")
...
function PLUGIN:TraceEyes( netuser ) 
	cs.registerstaticmethod( "tmpTrace", Raycast )
	local RaycastAll = tmpTrace
	tmpTrace = nil
